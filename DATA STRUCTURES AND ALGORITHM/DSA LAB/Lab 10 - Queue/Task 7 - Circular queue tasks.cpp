...
public:
    ...
    void averageOfKElems(int k) {
        ...
        if (count > 0) cout << "[Last Group]: " << (sum / (float)count);
        cout << endl;
    }
    ...
    void mergeWithoutDuplicates(CircularQueue& other) {
        ...
        if (temp) {
            do {
                if (seen.find(temp->data) == seen.end()) enqueue(temp->data);
                temp = temp->next;
    ...
    void customSkipFactor(int skip) {
        ...
        while (curr && curr->next != curr) {
            ...
            if (delNode == rear) rear = curr;
            if (delNode == front) front = front->next;
